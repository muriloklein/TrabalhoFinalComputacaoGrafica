# 🚀 Space Shooter – Trabalho Final

Jogo 2D desenvolvido em Unity para a disciplina de **Computação Gráfica (UPF)**.  
Controle uma nave, desvie de asteroides, destrua inimigos, avance até a linha final e tente entrar no **Leaderboard online**.

## 🎮 Funcionalidades

- Movimentação com física + turbo (combustível limitado)
- Inimigos com IA: perseguição, flanqueamento, ondas e rajadas de tiro
- Sistema de danos, explosões, shake de câmera e efeitos sonoros
- Asteroides com colisão precisa (PolygonCollider2D)
- Condição de vitória (precisa matar 5 inimigos antes da linha final)
- UI completa: combustível, kills, game over, vitória, leaderboard

## 📦 Download

**Executável (.exe):**  
➡ [Exe](https://drive.google.com/file/d/1Gf8E-5rdL9Z0aE6poefaB13O1GB6yP11/view?usp=sharing)

## 🎥 Demonstração

**Vídeo:**  
➡ [Demo](https://drive.google.com/file/d/1miQvRXxNOv6PZCvELq7msZLHl7VImGev/view?usp=sharing)

## ▶ Como Jogar

- **WASD** – mover  
- **Espaço** – atirar  
- **Enter** – turbo  
- **Esc** – menu  

## 🛠 Tecnologias

- Unity 2022+
- C#
- TextMeshPro
- Dan’s Leaderboard API

## 👨‍💻 Autores

**Guilherme Silveira Machado**  
Ciência da Computação – UPF
**Murilo Klein**  
Ciência da Computação – UPF
**Samuel Vitor Zibetti**  
Ciência da Computação – UPF

## 📄 Licença

Uso livre para fins educacionais.
